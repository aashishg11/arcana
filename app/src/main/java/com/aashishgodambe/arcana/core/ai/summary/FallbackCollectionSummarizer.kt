...
/**
 * Tries the showcase [MlKitCollectionSummarizer] (ML Kit GenAI Summarization — the 8th `ai-samples`
 * capability) first; if it can't run — feature 2004 not yet provisioned, or any error — transparently
 * falls back to [GeminiCollectionSummarizer] (the proven Gemini Nano path). Ships the new capability
 * whenever it's available and degrades gracefully to the other on-device engine otherwise.
 *
 * ML Kit failures surface up front (a status check), before any tokens stream, so the fallback swaps in
 * cleanly. This is the [CollectionSummarizer] seam doing what a seam is for.
 */
class FallbackCollectionSummarizer @Inject constructor(
    private val primary: MlKitCollectionSummarizer,
    private val fallback: GeminiCollectionSummarizer,
